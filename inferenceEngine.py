...
class InferenceEngine:
    ...
    def determine_next_action(self, current_pos: Tuple[int, int], percepts: List[str], grid_size: int) -> str:
        ...
        # Check if arrow should be used - Enhanced logic
        if self.kb.has_arrow:
            arrow_targets = self.kb.get_arrow_targets(current_pos)
            if arrow_targets:
                # Prioritize shooting in these scenarios:
                # 1. Definite wumpus (confidence = 1.0)
                # 2. High confidence wumpus (>= 0.8) and no safe path
                # 3. High confidence wumpus (>= 0.7) blocking exploration
                
                definite_wumpus = [pos for pos in arrow_targets if self.kb.get_confidence(pos, 'wumpus') == 1.0]
                high_conf_wumpus = [pos for pos in arrow_targets if self.kb.get_confidence(pos, 'wumpus') >= 0.8]
                blocking_wumpus = [pos for pos in arrow_targets if self.kb.get_confidence(pos, 'wumpus') >= 0.7]
                ...
                if definite_wumpus:
                    ...
                    reason = f"Shooting definite wumpus at {target}"
                elif high_conf_wumpus and not self.kb.can_reach_unvisited_safely(current_pos):
                    target = max(high_conf_wumpus, key=lambda pos: self.kb.get_confidence(pos, 'wumpus'))
                    reason = f"No safe path and high confidence wumpus at {target} (confidence: {self.kb.get_confidence(target, 'wumpus'):.2f})"
                elif blocking_wumpus:
                    # Check if any blocking wumpus is preventing access to unvisited areas
                    for wumpus_pos in blocking_wumpus:
                        # Temporarily assume wumpus is killed to see if it opens new paths
                        original_conf = self.kb.get_confidence(wumpus_pos, 'wumpus')
                        self.kb.set_confidence(wumpus_pos, 'wumpus', 0.0)  # Temporarily remove wumpus
                        can_reach_after = self.kb.can_reach_unvisited_safely(current_pos)
                        self.kb.set_confidence(wumpus_pos, 'wumpus', original_conf)  # Restore
                        
                        if can_reach_after:
                            target = wumpus_pos
                            reason = f"Shooting blocking wumpus at {target} to open exploration path (confidence: {original_conf:.2f})"
                            break
                ...
                if target:
                    ...
                    self.last_inference = f"WumpusTarget → ShootArrow_{direction}"
                    logger.info(f"Shooting arrow at {target} from {current_pos}: {reason}")
